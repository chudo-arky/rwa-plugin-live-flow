...
| форма выходов, каскады | конфиг-профили панели + сниппеты (`{"snippet": "имя"}` раскрываются) | кеш 60 с (при ошибке профилей или сниппетов — последний удачный набор с пометкой `profiles_stale`; на холодном старте без панели — `profiles_available: false`) |
...
**Сниппеты и недоступность конфигурации.** Аутбаунды, вынесенные в сниппеты панели,
раскрываются при загрузке профилей. Если панель не ответила на запрос профилей или сниппетов,
