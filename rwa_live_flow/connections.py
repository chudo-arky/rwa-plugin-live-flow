...
class ConnectionsPoller:
    ...
    def __init__(self) -> None:
        # uuid → {phase: idle|posted, job, posted_at, ips, as_of, fails}
    ...
    def ips(self, node_uuid: str) -> dict[str, list[str]] | None:
        """``{id пользователя: [IP, …]}`` или None, если снимка ещё нет.
    ...
    def age_s(self, node_uuid: str) -> float | None:
        st = self._st.get(node_uuid) or {}
        return None if st.get("as_of") is None else time.time() - st["as_of"]
    ...
    async def _tick_impl(self, nodes: dict[str, dict], log: Any) -> None:
        ...
        for uuid in [u for u in live if (self._st.get(u) or {}).get("phase") == "posted"]:
            ...
            if got is None:
                ...
                if now - st.get("posted_at", now) > JOB_TTL_S:
                    st.update(phase="idle", job=None)
                continue
            st.update(phase="idle", job=None, ips=got, as_of=now, fails=0)
        ...
        for uuid in order:
            if posted >= NODES_PER_TICK or inflight >= MAX_INFLIGHT:
                break
            ...
            if jid:
                ...
                inflight += 1
            posted += 1
        self._cursor = (self._cursor + posted) % max(1, len(live))
        self.error = None
        self.failures = 0
